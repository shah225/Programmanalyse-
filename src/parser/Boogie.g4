...
while_cmd
    :   'while' guard ( invariant )* LBrace stmt_list RBrace
    ;

invariant
    :   'free'? 'invariant' attr* expr Semi
    ;
...
rel_op  //returns [std::shared_ptr<COpBin> op]
    :   '=='		//{$op = std::make_shared<COpBin>("==");}
    |   '!='	 //	{$op = std::make_shared<COpBin>("!=");}
    |   '<='	 //	{$op = std::make_shared<COpBin>("<=");}
    |   '>='	 //	{$op = std::make_shared<COpBin>(">=");}
    |   Less	 //	{$op = std::make_shared<COpBin>("<");}
    |   Greater	 //	{$op = std::make_shared<COpBin>(">");}
    |	'<:'
    ;
